...
// GetUnsplashWallpaper returns a random Unsplash image URL through proxy
...
func GetUnsplashWallpaper(c *gin.Context) {
	...
	cached, exists := unsplashWallpaperCache[query]
	...
	if err != nil {
		log.Printf("Error fetching Unsplash redirect: %v", err)
		// Fallback: return the redirect URL directly
		// Still cache it for a short time to avoid repeated failures
		if !exists || cached == nil {
			unsplashWallpaperCacheMu.Lock()
			unsplashWallpaperCache[query] = &WallpaperCacheEntry{
				URL:       imageURL,
				Timestamp: time.Now(),
			}
	...
	if resp.StatusCode == http.StatusFound || resp.StatusCode == http.StatusMovedPermanently {
		...
		// Sometimes Unsplash returns the image directly
		finalURL = imageURL
	} else {
		log.Printf("Unsplash returned unexpected status: %d", resp.StatusCode)
